...
Every tool imports get_llm_client() from here instead of
building its own OpenAI client. This is the single place
to add new providers, rotate keys, or handle rate limits.
"""
import random
...
def get_llm_client() -> tuple[OpenAI, str]:
    ...
    Priority:
    1. Groq (free, fast) — rotates across all configured keys randomly.
    2. OpenAI (fallback if Groq keys are absent).
    """
